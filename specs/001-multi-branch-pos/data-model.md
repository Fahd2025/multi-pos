...
- `IX_Users_UserId` (on UserId)
- `IX_Users_BranchId` (on BranchId)
- `UQ_Users_UserBranch` (UNIQUE on UserId + BranchId)

**Validation Rules**:

- User cannot be assigned to same branch twice (enforced by unique constraint)
- `Role` must be one of: 0, 1, 2
- Head office admins (IsHeadOfficeAdmin=TRUE) don't need User entries
...
- User assignments (when users added/removed from branch)
