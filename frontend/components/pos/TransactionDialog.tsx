    ...
    try {
      ...
      const saleData = {
        ...
        ...(orderType === "delivery" && {
          ...
          specialInstructions: driver.driverName ? `Driver: ${driver.driverName}` : undefined,
        }),
      };
        ...
        if (!template) {
          ...
          setTimeout(() => {
            ...
            if (invoiceRef.current && handlePrint) {
              ...
              setTimeout(async () => {
                // If this is a delivery order, create the associated delivery order record
                if (orderType === "delivery") {
                  try {
                    const deliveryOrderData = {
                      orderId: sale.id,
                      customerId: sale.customerId, // Get customer ID from sale if available, undefined if not
                      pickupAddress: "", // Could be the branch address, would need to get from branch context
                      deliveryAddress: customer.address,
                      deliveryLocation: "", // Could be GPS coordinates if available
                      priority: 1, // 1 = Normal (corresponds to DeliveryPriority.Normal)
                      specialInstructions: `Customer: ${customer.name} | Phone: ${customer.phone}`,
                      estimatedDeliveryMinutes: 45, // Default estimate
                    };

                    await deliveryService.createDeliveryOrder(deliveryOrderData);
                    console.log("Delivery order created successfully for sale:", sale.id);
                  } catch (deliveryError) {
                    console.error("Failed to create delivery order:", deliveryError);
                    toast.error(
                      "Delivery Order Error",
                      "Sale completed but delivery order creation failed"
                    );
                    // Still proceed to close because the sale was successful
                  }
                }
              ...
              console.error(
                "Cannot print - invoiceRef.current:",
                invoiceRef.current,
                "handlePrint:",
                handlePrint
              );
