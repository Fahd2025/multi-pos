...
export default function SuppliersPage() {
  ...
  const createEditModal = useModal<SupplierDto>();
  ...
  const handleSortChange = (config: { key: keyof SupplierDto | string; direction: 'asc' | 'desc' }) => {
  ...
  // Define form fields
  const formFields: FormField<SupplierDto>[] = [
    {
      name: 'code',
      label: 'Supplier Code',
      type: 'text',
  ...
  const handleSubmit = async (data: SupplierDto) => {
    ...
    try {
      if (createEditModal.mode === 'create') {
        ...
        await supplierService.updateSupplier(data.id, updateData);
      }
  ...
  const handleDeleteClick = (supplier: SupplierDto) => {
    ...
    const message = hasHistory
      ? `"${supplier.nameEn}" has ${supplier.totalPurchases} purchase(s) worth $${supplier.totalSpent.toFixed(2)}. The supplier will be marked as inactive instead of being permanently deleted.`
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      ...
      <ModalBottomSheet
        ...
        title={createEditModal.mode === 'create' ? 'Add New Supplier' : 'Edit Supplier'}
