...
- Changed `showBranchName` → `showBranchName` (terminology update)
...
- Changed "Show Branch Name" → "Show Branch Name"
- Changed `showBranchName` → `showBranchName`
...
- Updated to support both `showBranchName` and `showBranchName` (backward compatibility)
...
- Detects Arabic in branch name and customer name
...
const isRTL =
  schema.rtl !== undefined
    ? schema.rtl
    : hasArabicContent(data.branchNameAr) ||
      hasArabicContent(data.customerName);
```
  ...
  schema && (
    <div className="flex items-center">
      <input
        ...
        onChange={(e) =>
          setSchema((prev) =>
            prev ? { ...prev, rtl: e.target.checked } : prev
          )
        }
      />
      <label htmlFor="rtl-toggle">
...
_This progress document tracks the implementation of the comprehensive invoice builder completion plan. It will be updated as work continues._
